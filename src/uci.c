...
void ParseGo(char *line)
{
    ...
    else if (time != -1)
    {
        ...
        if (timeToSpend > 10000)
            timeToSpend = 10000; // cap at 10s for now to be safe
    }
    else
    {
...
void ParsePosition(char *lineIn)
{
    ...
    if (ptrChar != NULL)
    {
        ...
        while (*ptrChar)
        {
            ...
            // parse algebraic move from UCI string
            ...
            for (int i = 0; i < list.count; i++)
            {
                ...
                if (m.from == from && m.to == to)
                {
                    ...
                    if (m.flags & FLAG_PROMOTION)
                    {
                        ...
                        if (promChar == 'q')
                            promPiece = (side == WHITE) ? Q : q;
                        else if (promChar == 'r')
                            promPiece = (side == WHITE) ? R : r;
                        else if (promChar == 'b')
                            promPiece = (side == WHITE) ? B : b;
                        else if (promChar == 'n')
                            promPiece = (side == WHITE) ? N : n;

                        // if the generated move is not the promotion we want, skip
                        if (m.promotion != promPiece)
                            continue;
                    }
            ...
            // moveFound to verify input safety
            if (!moveFound)
            {
                printf("info string Error: Unknown move encountered: %c%c%c%c\n",
                       ptrChar[0], ptrChar[1], ptrChar[2], ptrChar[3]);
                break; // stop parsing to prevent crashes
            }
